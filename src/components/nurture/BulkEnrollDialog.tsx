...
export function BulkEnrollDialog({ open, onOpenChange, sequences, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        ...
        .eq("id", sequenceId)
        .maybeSingle();
      const ft = data?.filter_tags || [];
      if (ft.length) {
        setTags(ft);
        setTagsMode(data?.filter_tags_mode === "all" ? "all" : "any");
      }
    })();
  }, [sequenceId]);

  const filteredQuery = (head = false) => {
    let q = supabase.from("imphq_leads").select("id, email", { count: "exact", head });
    if (produto.trim()) q = q.ilike("data->>ultimo_produto", `%${produto.trim()}%`);
    if (tags.length) q = tagsMode === "all" ? q.contains("tags", tags) : q.overlaps("tags", tags);
  ...
  const preview = async () => {
    setBusy(true);
    try {
      const { count, error } = await filteredQuery(true);
  ...
  const enroll = async () => {
    ...
    setBusy(true);
    try {
      const { data: leads, error } = await filteredQuery().limit(2000);
      if (error) throw error;
      ...
      const { error: insErr } = await supabase.from("imphq_lead_sequence_enrollments").insert(rows);
      ...
      <DialogContent className="max-w-lg bg-secondary/40">
        ...
        <div className="space-y-3 leading-7">
          ...
          {tags.length > 1 && (
            <div>
              <Label>Modo</Label>
              <Select value={tagsMode} onValueChange={(v) => setTagsMode(v === "all" ? "all" : "any")}>
