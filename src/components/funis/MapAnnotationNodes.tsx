import { sortScheduleItems, DEFAULT_SCHEDULE_ITEMS, detectReelPlatform, extractReelAuthor, extractReelThumb, type AnnotationKind, type ScheduleItemKind, type ScheduleItem, type AnnotationData } from "@/components/funis/map-annotation-data";
...
export const AnnotationReelNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const writeClipboard = async (text: string): Promise<boolean> => {
    try {
      ...
      // Clipboard permissions can reject this API; the textarea fallback below still works.
    }
...
function GeneratorShell({
  ...
  icon: LucideIcon; kindLabel: string; accent: string; minH?: number;
...
// -------- Schedule / Cronograma --------
const SCHEDULE_ITEM_META: Record<ScheduleItemKind, { label: string; color: string; Icon: LucideIcon }> = {
...
// ============================================================
const ACCOUNT_CACHE = new Map<string, Tables<"imphq_empresa">>();
...
export const AnnotationAccountNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const [row, setRow] = useState<Tables<"imphq_empresa"> | null>(accountId ? ACCOUNT_CACHE.get(accountId) : null);
