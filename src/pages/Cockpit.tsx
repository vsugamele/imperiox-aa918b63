...
export default function Cockpit() {
  ...
  return (
    <div className="w-full min-h-screen">
      {/* ── BARRA GLOBAL DE SELEÇÃO DE MODO TRI-MODO ──────────────────────── */}
      <div className="sticky top-0 z-50 flex items-center justify-between px-4 py-2 text-xs font-mono border-b backdrop-blur-md transition-colors duration-300"
        style={{
          background: theme === "papel" ? "#E2DED5" : theme === "terminal" ? "#0D0F12" : "#0A0B0D",
          borderColor: theme === "papel" ? "#C9C4B8" : theme === "terminal" ? "#16191E" : "#1B1E23",
          color: theme === "papel" ? "#14120F" : theme === "terminal" ? "#D7DBE0" : "#E8EAED",
        }}
      >
        <div className="flex items-center gap-3">
          <span className="font-bold tracking-widest text-[10px] uppercase opacity-70">
            VISÃO COCKPIT:
          </span>
          <div className="inline-flex rounded border p-0.5 gap-1"
            style={{
              borderColor: theme === "papel" ? "#B5AEA2" : theme === "terminal" ? "#2A2F37" : "#23262C",
              background: theme === "papel" ? "#EDEAE3" : theme === "terminal" ? "#07080A" : "#101215",
            }}
          >
            <button
              ...
              className={`px-3 py-1 text-[11px] font-medium rounded transition-all ${
                theme === "grafite" 
                  ? "bg-[#D6FF4B] text-[#0A0B0D] font-bold shadow-sm" 
            ...
            <button
              ...
              className={`px-3 py-1 text-[11px] font-medium rounded transition-all ${
                theme === "papel" 
                  ? "bg-[#D93A14] text-white font-bold shadow-sm" 
                  : "opacity-60 hover:opacity-100"
              }`}
            >
              📰 Papel Operacional
            ...
            <button
              ...
              className={`px-3 py-1 text-[11px] font-medium rounded transition-all ${
                theme === "terminal" 
                  ? "bg-[#FFB020] text-[#07080A] font-bold shadow-sm" 
                  : "opacity-60 hover:opacity-100"
              }`}
            >
              📟 Bloomberg Terminal
        ...
        <div className="flex items-center gap-3">
          <Link
            to="/redesign"
            className="px-2.5 py-1 rounded border text-[10px] font-mono tracking-wider transition-colors hover:border-current"
            style={{
              borderColor: theme === "papel" ? "#B5AEA2" : theme === "terminal" ? "#2A2F37" : "#23262C",
              color: theme === "papel" ? "#D93A14" : theme === "terminal" ? "#FFB020" : "#D6FF4B",
            }}
          >
            EXPLORAR TODAS AS TELAS DO REDESIGN →
          </Link>
          <button
            onClick={() => setZenMode(!zenMode)}
            className="flex items-center gap-1.5 px-2.5 py-1 rounded border text-[10px] font-mono tracking-wider transition-colors"
            style={{
              borderColor: theme === "papel" ? "#B5AEA2" : theme === "terminal" ? "#2A2F37" : "#23262C",
              background: zenMode ? (theme === "papel" ? "#14120F" : theme === "terminal" ? "#FFB020" : "#D6FF4B") : "transparent",
              color: zenMode ? (theme === "papel" ? "#EDEAE3" : "#0A0B0D") : "inherit",
            }}
          >
      ...
      {/* ────────────────────────────────────────────────────────────────────
          RENDERIZAÇÃO CONDICIONAL DO TEMA SELECIONADO
      ──────────────────────────────────────────────────────────────────── */}
      {theme === "grafite" && (
        <CockpitGrafiteView 
          barData={barData} 
...
/* ══════════════════════════════════════════════════════════════════════════════
   ...
   Paleta: #0A0B0D, Hairlines #1B1E23, Acento Lima #D6FF4B, Archivo + IBM Plex Mono
   ══════════════════════════════════════════════════════════════════════════════ */
function CockpitGrafiteView({
  barData,
  pausedAdv,
  onPauseAdv,
  zenMode,
  ...
  const queue = [
    { label: "3 leads quentes sem resposta há 20min", dot: "#FB7185", link: "/inbox" },
    { label: "Checkout Care caiu 18% desde ontem", dot: "#FBBF24", link: "/funis" },
    { label: "Saldo Meta cobre 6 dias", dot: "#FBBF24", link: "/gerenciador" },
    { label: "MemoFlow bateu meta do mês", dot: "#4ADE80", link: "/projetos" },
  ];
  ...
  return (
    <div className="bg-[#0A0B0D] text-[#E8EAED] font-['Archivo',sans-serif] min-h-screen px-4 py-6 md:px-8 max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-300">
      {/* HEADER DE BOLETIM */}
      <header className="flex flex-wrap items-baseline justify-between gap-4 border-b border-[#1B1E23] pb-5">
        <div>
          <div className="font-mono text-[9px] tracking-[0.22em] text-[#5F646D] mb-1.5 uppercase">
            PAINEL DE INSTRUMENTOS · HOJE
          </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded bg-[#101215] border border-[#1B1E23] font-mono text-[10px] text-[#8A8F98]">
            <span className="w-2 h-2 rounded-full bg-[#4ADE80] animate-pulse" />
            AO VIVO · 14 LEADS/H
          </div>
          <div className="font-mono text-[10px] text-[#5F646D] border border-[#1B1E23] px-3 py-1.5 rounded">
            ÚLTIMOS 30 DIAS
          </div>
        </div>
      </header>

      {/* SEÇÃO 1: A DECISÃO DE HOJE */}
      ...
      {/* SEÇÃO 2: CAIXA / KPIS EM GRID HAIRLINE */}
      ...
      {/* SEÇÃO 3: DOIS BLOCOS (RECEITA VS ADS & QUEM PAGA A CONTA) */}
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* GRÁFICO BARRAS DUPLAS 14D */}
            ...
            {barData.map((v, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col justify-end gap-0.5 h-full group relative">
                  ...
                  />
                  {/* Tooltip simples */}
                  <div className="opacity-0 group-hover:opacity-100 absolute -top-8 left-1/2 -translate-x-1/2 bg-[#1B1E23] px-2 py-1 rounded text-[9px] font-mono whitespace-nowrap pointer-events-none z-10 transition-opacity">
                    Dia {i + 1}: {v}%
                  </div>
                </div>
              );
      ...
      {/* SEÇÃO 4: SEGUNDO PLANO · SÓ CONSULTA */}
      {!zenMode && (
...
/* ══════════════════════════════════════════════════════════════════════════════
   2. MODO PAPEL OPERACIONAL (EDITORIAL NEWSPAPER)
   Paleta: #EDEAE3, Carvão #14120F, Acento #D93A14, Familjen Grotesk + Martian Mono
   ══════════════════════════════════════════════════════════════════════════════ */
function CockpitPapelView({
  barData,
  pausedAdv,
  onPauseAdv,
  zenMode,
  ...
  const queue = [
    { n: "01", label: "3 leads quentes sem resposta há 20min", color: "#D93A14", link: "/inbox" },
    { n: "02", label: "Checkout Care caiu 18% desde ontem", color: "#B8860B", link: "/funis" },
    { n: "03", label: "Saldo Meta cobre 6 dias", color: "#B8860B", link: "/gerenciador" },
    { n: "04", label: "MemoFlow bateu meta do mês", color: "#2E7D52", link: "/projetos" },
  ];
  ...
  return (
    <div className="bg-[#EDEAE3] text-[#14120F] font-['Familjen_Grotesk',sans-serif] min-h-screen px-4 py-8 md:px-10 max-w-[1340px] mx-auto space-y-8 animate-in fade-in duration-300">
      {/* HEADER DE JORNAL */}
      <header className="flex flex-wrap items-end justify-between gap-4 border-b-2 border-[#14120F] pb-4">
        <div>
          <div className="font-['Martian_Mono',monospace] text-[9px] tracking-[0.24em] text-[#8C857A] mb-1.5 uppercase">
            BOLETIM DIÁRIO DA OPERAÇÃO · 18 SET 2026 · 17:40
          </div>
            ...
            <div className="flex flex-col divide-y divide-[#C9C4B8]">
              {queue.map((q, i) => (
                <Link
                  key={i}
                  to={q.link}
                  className="grid grid-cols-[20px_1fr] gap-2 py-2.5 hover:opacity-75 transition-opacity"
                >
                  <span
                    className="font-['Martian_Mono',monospace] text-[9px] font-bold"
                    style={{ color: q.color }}
                  >
                    {q.n}
                  </span>
                  <span className="text-xs text-[#4A443B] leading-snug">
                    {q.label}
                  </span>
                </Link>
              ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-px bg-[#C9C4B8]">
          {kpis.map((k, i) => (
            <div key={i} className="bg-[#EDEAE3] p-4 min-w-0">
              ...
              <div
                className="text-2xl font-bold tracking-tight leading-none mb-1.5 truncate"
                style={{ color: k.color }}
              >
                {k.value}
              </div>
              <div
                className="font-['Martian_Mono',monospace] text-[10px] font-medium"
                style={{ color: k.deltaColor }}
              >
                {k.delta}
              </div>
      ...
      {/* SEÇÃO 3: RECEITA VS ADS E PROJETOS */}
              ...
              return (
                <div key={i} className="flex-1 flex flex-col justify-end gap-0.5 h-full">
                  <div
                    ...
                    style={{
                      height: `${rev}%`,
                      background: isLast ? "#D93A14" : "#14120F",
                    }}
                  />
                  <div
                    className="w-full bg-[#C9C4B8]"
                    style={{ height: `${ads}%` }}
                  />
                </div>
              );
          ...
          <div className="divide-y divide-[#C9C4B8]">
            {projects.map((p, i) => (
              <div key={i} className="grid grid-cols-[1fr_96px_62px] items-center gap-3 py-2.5">
                ...
                <div
                  className="font-['Martian_Mono',monospace] text-xs text-right font-bold"
                  style={{ color: p.roasColor }}
                >
                  {p.roas}
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-6 gap-6">
            {minor.map((m, i) => (
              <div key={i}>
                ...
                <div className="text-sm font-semibold text-[#4A443B]">
                  {m.value}
                </div>
              </div>
            ))}
...
/* ══════════════════════════════════════════════════════════════════════════════
   3. MODO TERMINAL BLOOMBERG (HIGH-DENSITY CYBER-OPERATOR)
   Paleta: #07080A, Âmbar #FFB020, Alerta #F4526A, Lucro #3ED598, JetBrains Mono
   ══════════════════════════════════════════════════════════════════════════════ */
function CockpitTerminalView({
  barData,
  pausedAdv,
  onPauseAdv,
  zenMode,
  ...
  const queue = [
    { sev: "P1", label: "3 leads quentes sem resposta há 20min", color: "#F4526A", link: "/inbox" },
    { sev: "P2", label: "Checkout Care −18% vs. ontem", color: "#FFB020", link: "/funis" },
    { sev: "P2", label: "Saldo Meta cobre 6 dias", color: "#FFB020", link: "/gerenciador" },
    { sev: "OK", label: "MemoFlow bateu meta do mês", color: "#3ED598", link: "/projetos" },
  ];
  ...
  return (
    <div className="bg-[#07080A] text-[#D7DBE0] font-['JetBrains_Mono',monospace] min-h-screen animate-in fade-in duration-300">
      {/* TICKER DE ALTA DENSIDADE */}
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-6 space-y-6">
        {/* SEÇÃO 1: ALERTA CRÍTICO DE AÇÃO */}
          ...
          <div className="flex flex-wrap lg:flex-nowrap gap-6 p-4 md:p-5">
            <div className="flex-1 min-w-[280px]">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-4">
                ...
                <Link
                  to="/gerenciador"
                  className="h-8 px-4 border border-[#2A2F37] text-xs tracking-wider text-[#D7DBE0] hover:border-[#FFB020] hover:text-[#FFB020] flex items-center transition-colors"
                >
                  [ VER CRIATIVO ]
                </Link>
                <button 
                  onClick={() => toast.info("Snooze ativo por 24h.")}
                  className="h-8 px-3 text-[10px] tracking-widest text-[#4E555F] hover:text-[#8C939C]"
                >
                  SNOOZE 24H
                </button>
              </div>
              ...
              <div className="divide-y divide-[#16191E]">
                {queue.map((q, i) => (
                  <Link
                    key={i}
                    to={q.link}
                    className="grid grid-cols-[30px_1fr] gap-2 py-2 hover:bg-[#0F1216] transition-colors"
                  >
                    <span className="text-[9px] font-bold" style={{ color: q.color }}>
                      {q.sev}
                    </span>
                    <span className="text-[11px] text-[#A8AEB6] leading-snug">
                      {q.label}
                    </span>
                  </Link>
                ))}
        ...
        {/* SEÇÃO 2: RECEITA/ADS E PROJETOS EM GRID */}
          ...
          <div>
            ...
            <div className="p-1">
              ...
              <div className="divide-y divide-[#0F1216]">
                {projects.map((p, i) => (
                  <div key={i} className="grid grid-cols-[1fr_90px_60px_50px] gap-2 px-3 py-2 text-[11px] hover:bg-[#0F1216]">
