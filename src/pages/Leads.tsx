...
export default function Leads() {
  ...
  const loadTimeline = async (lead: Lead) => {
    ...
    if (lead.email) {
      // Semana 2: filtrar por project_id quando disponível + narrow select (corta egress significativo)
      ...
      promises.push(csvQ.order("created_at", { ascending: false }).limit(50).then(({ data }) => { (data || []).forEach((e: any) => { const evData = e.event_data || {}; events.push({ id: e.id, type: "CSVImport", timestamp: e.created_at, title: `Importado via ${evData.plataforma || "CSV"}`, subtitle: evData.produto ? `Produto: ${evData.produto}` : undefined, details: { status: evData.status_evento, pagamento: evData.metodo_pagamento, valor: evData.valor ? `R$ ${evData.valor}` : undefined, data_pedido: evData.data_pedido } }); }); }));
      const leadUtmSource = lead.data?.utms?.utm_source;
      if (leadUtmSource) {
        promises.push(supabase.from("imphq_clicks").select("id, page_url, created_at, utm_source, utm_medium, utm_campaign").eq("utm_source", leadUtmSource).order("created_at", { ascending: false }).limit(50).then(({ data }) => { (data || []).forEach((c: any) => { events.push({ id: c.id, type: "click", timestamp: c.created_at, title: "Click UTM", subtitle: c.page_url ? new URL(c.page_url).pathname : c.utm_campaign, details: { utm_source: c.utm_source, utm_medium: c.utm_medium, utm_campaign: c.utm_campaign } }); }); }));
      }
    }
