...
const stageMeta: Record<StageId, { title: string; pt: string; risk: string }> = {
  ...
  close: {
    title: "7. Fechamento dos Kits (Checkout Oficial)",
    pt: "Apresenta os kits oficiais da campanha com frete grátis, ancorando no Kit 6 ($19.99/frasco com 3 bônus) e garantia de 60 dias.",
    risk: "Usar sempre fechamento por dupla alternativa (ex: Kit 6 vs Kit 2).",
  },
  followup: {
...
const CHECKOUT_KITS = [
  {
    id: "kit2",
    name: "2 Jars (Buy 1 Get 1 Free) - Starter",
    namePt: "Kit 2 Frascos (1 + 1 Grátis) - Entrada",
    supply: "60 days",
    total: "$59.98",
    perJar: "$29.99 / jar",
    url: "https://slimsodapowder.com/cc2/dtc/pay/checkout.php?package=2b29aj&hid=b2lkPW9mZl81MDU4NzI1JmFpZD1hZmZfNjgyMTM3NyZ1aWQ9YmxfNzA0MTEyNQ%3D%3D&affid=aff_6821377",
    recommended: false,
  },
  {
    id: "kit4",
    name: "4 Jars (Buy 2 Get 2 Free) - Most Popular",
    namePt: "Kit 4 Frascos (2 + 2 Grátis) - Intermediário",
    supply: "120 days",
    total: "$99.96",
    perJar: "$24.99 / jar",
    url: "https://slimsodapowder.com/cc2/dtc/pay/checkout.php?package=4b24aj&hid=b2lkPW9mZl81MDU4NzI1JmFpZD1hZmZfNjgyMTM3NyZ1aWQ9YmxfNzA0MTEyNQ%3D%3D&affid=aff_6821377",
    recommended: false,
  },
  {
    id: "kit6",
    name: "6 Jars (Buy 3 Get 3 Free) - BEST VALUE",
    namePt: "Kit 6 Frascos (3 + 3 Grátis) - Campeão + 3 Bônus",
    supply: "180 days (Full Protocol)",
    total: "$119.94",
    perJar: "$19.99 / jar (Free Shipping)",
    url: "https://slimsodapowder.com/cc2/dtc/pay/checkout.php?package=6b19aj&hid=b2lkPW9mZl81MDU4NzI1JmFpZD1hZmZfNjgyMTM3NyZ1aWQ9YmxfNzA0MTEyNQ%3D%3D&affid=aff_6821377",
    recommended: true,
  },
];

const PRESELL_QUIZ_URL = "https://cc.slimsodapowder.com/dtcnew-whop/?hid=b2lkPW9mZl81MDU4NzI1JmFpZD1hZmZfNjgyMTM3NyZ1aWQ9YmxfNzc4NDk1OA%3D%3D&affid=aff_6821377";
...
function responseFor(stage: StageId, lead: LeadState, lang: Lang): string {
  ...
  if (lang === "pt") {
    if (stage === "spin") {
      return `Te entendo perfeitamente, ${name}. Quando você diz que sente "${lead.symptom}", saiba que a culpa NUNCA foi da sua força de vontade.\n\nO que a maioria das mulheres não sabe é que dietas e injeções apenas mascaram o sintoma. Me conta: quando você tentou ${lead.tried || "emagrecer no passado"}, você sentia que a fome voltava ainda mais feroz assim que dava uma pausa?`;
    }
    if (stage === "mechanism") {
    ...
    if (stage === "close") {
      return `Maravilha, ${name}! Hoje estamos com o lote reservado com desconto especial de fábrica e Frete Grátis em todos os pacotes:\n\n🏆 Kit 6 Frascos (Tratamento Completo 180 dias - BEST VALUE):\nde $588 por apenas $119.94 (cada frasco sai por apenas $19.99 com Frete Grátis prioritário + 3 Bônus Exclusivos de Rosto e Cintura). É a escolha de 90% das mulheres para resetar o corpo sem rebote.\n\n📦 Kit 4 Frascos (120 dias - Mais Popular): $99.96 ($24.99/frasco)\n📦 Kit 2 Frascos (60 dias - Entrada): $59.98 ($29.99/frasco)\n\n👉 Acesse o Checkout Seguro Oficial aqui:\nhttps://slimsodapowder.com/cc2/dtc/pay/checkout.php?package=6b19aj&hid=b2lkPW9mZl81MDU4NzI1JmFpZD1hZmZfNjgyMTM3NyZ1aWQ9YmxfNzA0MTEyNQ%3D%3D&affid=aff_6821377\n\nVocê prefere garantir o Kit de 6 Frascos com o maior desconto ou prefere iniciar com o de 2 Frascos?`;
    }
    return `${name}, passando só pra checar se você conseguiu abrir o link direitinho ou se teve alguma dúvida no checkout? Estamos com as últimas 56 unidades desse lote reservadas para você com frete grátis liberado!`;
  }

  // English fallback
  if (stage === "spin") {
    return `I hear you loud and clear, ${name}. When you mention "${lead.symptom}", please know this was NEVER about a lack of discipline.\n\nWhat Big Pharma never tells women over 40 is that diets and weekly injections only silence symptoms temporarily. When you tried ${lead.tried || "dieting in the past"}, did you feel like the food noise and hunger rebounded the moment you stopped?`;
  }
  if (stage === "mechanism") {
    return `That's the trap, ${name}. Your body already produces the exact same satiety hormones that medications copy: GLP-1 and GIP. They are made right inside your gut's L-cells.\n\nHowever, after 40, chronic stress and gut acidity put those L-cells to sleep. On top of that, an enzyme called DPP-4 destroys natural GLP-1 within minutes. When you stop the weekly injections, your body has forgotten how to produce it on its own.\n\nThat is why you felt trapped: not because you failed, but because your body's metabolic switch got stuck in storage mode.`;
  }
  if (stage === "proof") {
  ...
  if (stage === "temperature") {
    return `Before I share the reserved batch links, quick temperature check, ${name}:\n\nOn a scale of 0 to 10, how much does reactivating your body's own fat-burning hormones make sense for your journey right now?`;
  }
  if (stage === "objection") {
    return `I completely respect your caution, ${name}. Addressing your concern about "${lead.objection}":\n\n• Walmart baking soda won't work: regular baking soda only changes mouth/stomach pH for 15 minutes. It lacks bioactive berberine, standardized gingerol, and NAD+ to trigger AMPK.\n• Amazon berberine has <1% absorption: swallowing raw capsules breaks down before reaching L-cells. A dissolvable alkaline powder absorbs immediately.\n• 60-Day Money Back Guarantee: You get a full 60 days to test it. If your food noise doesn't disappear and your clothes don't loosen up, you get a 100% refund. No questions asked.`;
  }
  if (stage === "close") {
    return `Awesome, ${name}! Here are today's factory-direct discounted bundles with Free Shipping:\n\n🏆 6 Jars (Buy 3 Get 3 Free - BEST VALUE - 180-Day Supply):\nDown from $588 to just $119.94 ($19.99/jar with FREE Shipping + 3 Exclusive Bonuses: Runway Waist Formula, Youth Glow Masterclass & Zoom Consultation). 90% of customers choose this to reset their metabolism permanently without rebounds.\n\n📦 4 Jars (Buy 2 Get 2 Free - Most Popular): $99.96 ($24.99/jar)\n📦 2 Jars (Buy 1 Get 1 Free - Starter): $59.98 ($29.99/jar)\n\n👉 Secure Official Checkout Link:\nhttps://slimsodapowder.com/cc2/dtc/pay/checkout.php?package=6b19aj&hid=b2lkPW9mZl81MDU4NzI1JmFpZD1hZmZfNjgyMTM3NyZ1aWQ9YmxfNzA0MTEyNQ%3D%3D&affid=aff_6821377\n\nWould you like to lock in the 6-Jar Best Value bundle with free shipping, or start with the 2-Jar kit?`;
  }
  return `Hi ${name}, just checking in real quick to see if the checkout link opened smoothly for you or if you had any questions? We are down to the final 56 bottles reserved for you with free shipping!`;
}
...
const auditItems = [
  ...
  { ok: true, text: "Links oficiais de checkout cc2/dtc/pay integrados para 2, 4 e 6 potes com frete grátis." },
  { ok: true, text: "Página de presell/quiz oficial dtcnew-whop cadastrada no hub de produtos." },
      ...
      <section className="grid gap-5 xl:grid-cols-[330px_minmax(0,1fr)_400px]">
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-xl border border-border/60 bg-card/60 p-4 shadow-sm">
            <div className="flex items-center gap-2 border-b border-border/40 pb-3">
              ...
              <h2 className="text-sm font-semibold tracking-wide uppercase text-foreground">
                {lang === "pt" ? "Kits Oficiais SlimSoda Pay" : "Official SlimSoda Checkout Bundles"}
              </h2>
            </div>
            <div className="mt-3 space-y-2.5">
              ...
              <div className="pt-2 border-t border-border/40 mt-3">
                <div className="flex items-center justify-between text-[11px] text-muted-foreground mb-1">
                  <span>{lang === "pt" ? "Página Advertorial / Quiz:" : "Presell Advertorial / Quiz:"}</span>
                </div>
                <a
                  href={PRESELL_QUIZ_URL}
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center justify-between p-2 rounded border border-border/60 bg-background/40 hover:bg-background/80 text-xs text-primary truncate"
                >
                  <span className="truncate">cc.slimsodapowder.com/dtcnew-whop</span>
        ...
        <main className="flex min-h-[760px] flex-col rounded-xl border border-border/60 bg-card/50 shadow-sm overflow-hidden">
          ...
          <div className="flex-1 space-y-4 overflow-y-auto p-4 md:p-5">
            {messages.map((m) => (
              <div key={m.id} className={`flex ${m.sender === "lead" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-4 py-3 text-sm leading-relaxed shadow-sm ${
                    m.sender === "lead"
                      ? "rounded-br-none bg-primary text-primary-foreground font-medium"
                      : "rounded-bl-none border border-border/70 bg-card/90 text-foreground"
                  }`}
                >
                  <div
                    className={`mb-1.5 flex items-center gap-1.5 text-[10px] uppercase tracking-wider ${
                      m.sender === "lead" ? "text-primary-foreground/75" : "text-primary font-semibold"
                    }`}
                  >
              ...
              {(Object.keys(stageMeta) as StageId[]).map((st, idx) => (
                <div
                  ...
                  className={`rounded-lg border px-3 py-2 text-xs transition-all ${
                    st === currentStage
                      ? "border-primary/60 bg-primary/15 text-foreground font-semibold ring-1 ring-primary/30"
                      : "border-border/50 bg-background/40 text-muted-foreground opacity-80"
                  }`}
                >
