...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (row.provider !== "kie" || !row.external_id) {
    ...
    const resultUrl =
      data?.data?.resultJson && (() => { try { return JSON.parse(data.data.resultJson)?.resultUrls?.[0]; } catch { return null; } })() ||
    ...
    if (status === "success" || status === "succeeded" || resultUrl) {
      ...
      const publicUrl = await uploadFromUrl(supabase, userId, resultUrl, "mp4", "video/mp4");
