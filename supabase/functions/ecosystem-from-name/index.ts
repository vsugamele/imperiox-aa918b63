...
const CopyAngles = z.object({ criativos: z.array(z.object({ angulo: z.string(), headline: z.string(), hook: z.string(), body: z.string(), cta: z.string() }).passthrough()).nullish() }).passthrough();
const ReelsCopy = z.object({ reels: z.array(z.object({ estrutura: z.string(), hook: z.string(), roteiro_completo: z.string(), cta: z.string() }).passthrough()).nullish() }).passthrough();
const WaCopy = z.object({ mensagens: z.array(z.object({ ordem: z.number(), objetivo: z.string(), delay_minutos: z.number(), texto: z.string() }).passthrough()).nullish() }).passthrough();
...
Deno.serve(async (req) => {
  ...
  (async () => {
    try {
      ...
      let avatar: unknown = proj?.avatar || projData?.avatar || null;
      ...
      const resultado: { projeto_id: string; etapas: Record<string, unknown>; hub?: { assets: number }; audit?: unknown } = { projeto_id: projectId, etapas: {} };
      ...
      if (etapas.includes("avatar") && !(body.modo === "complementar" && temAvatarExistente)) {
        ...
        try {
          const av = record(await runSkill({
          ...
          resultado.etapas.avatar = { ok: false, error: String(errorMessage(e) || e) };
          emit({ type: "step_error", step: "avatar", error: String(errorMessage(e) || e) });
        }
      ...
      async function runStep(step: Step, fn: () => Promise<{ preview: string; hubAsset?: HubAssetSeed | null }>) {
        ...
        try {
          ...
          resultado.etapas[step] = { ok: false, error: String(errorMessage(e) || e) };
          emit({ type: "step_error", step, error: String(errorMessage(e) || e) });
        }
      }
      ...
      if (etapas.includes("vsl")) parallelJobs.push(runStep("vsl", async () => {
        const vsl = z.string().parse(await runSkill({
      ...
      if (etapas.includes("lp")) parallelJobs.push(runStep("lp", async () => {
        const lp = z.string().parse(await runSkill({
      ...
      if (etapas.includes("angulos")) parallelJobs.push(runStep("angulos", async () => {
        const angulos = CopyAngles.parse(await runSkill({
        ...
        const consolidado = (angulos.criativos || []).map((c, i: number) =>
          `## Ângulo ${i + 1}: ${c.angulo}\n**Headline:** ${c.headline}\n**Hook (3s):** ${c.hook}\n**Body:**\n${c.body}\n**CTA:** ${c.cta}`
        ).join("\n\n---\n\n");
        await sb.from("imphq_swipes").insert({
          user_id: userId, project_id: projectId,
      ...
      if (etapas.includes("reels")) parallelJobs.push(runStep("reels", async () => {
        const reels = ReelsCopy.parse(await runSkill({
        ...
        const consolidado = (reels.reels || []).map((r, i: number) =>
      ...
      if (etapas.includes("whatsapp_x1")) parallelJobs.push(runStep("whatsapp_x1", async () => {
        const wa = WaCopy.parse(await runSkill({
        ...
        const textoConsolidado = (wa.mensagens || []).sort((a,b)=>a.ordem-b.ordem).map((m) =>
      ...
      if (etapas.includes("fluxos_pos_venda")) {
        ...
        try {
          ...
          resultado.etapas.fluxos_pos_venda = { ok: false, error: String(errorMessage(e) || e) };
          emit({ type: "step_error", step: "fluxos_pos_venda", error: String(errorMessage(e) || e) });
        }
      }

      // ===== Garante produto no briefing + monta Hub no canvas =====
      try {
        emit({ type: "step_start", step: "hub" });
        ...
        const pd = record(projRow?.data);
        const briefing = (pd.briefing && typeof pd.briefing === "object") ? record(pd.briefing) : null;
        const target = briefing || pd;
        const lista: unknown[] = Array.isArray(target.produtos) ? target.produtos : [];
        const existe = lista.some((p) => (typeof p === "string" ? p : record(p).nome || record(p).name) === produto_nome);
        if (!existe) {
        ...
        if (hubRow) {
          const hub = record(record(hubRow.data).hub);
          hub[produto_nome] = hubAssets;
          await sb.from("imphq_funis").update({ data: { ...record(hubRow.data), hub } }).eq("id", hubRow.id);
        } else {
        ...
        emit({ type: "step_done", step: "hub", preview: `Hub montado com ${hubAssets.length} ativos posicionados` });
      } catch (e) {
        emit({ type: "step_error", step: "hub", error: String(errorMessage(e) || e) });
      }
      ...
      if (!body.skip_audit) {
        ...
        try {
          ...
          emit({ type: "audit_error", error: String(errorMessage(e) || e) });
        }
      }
      ...
      emit({ type: "fatal", error: String(errorMessage(e) || e) });
    } finally {
      writer.close().catch(() => {});
    }
  })();
